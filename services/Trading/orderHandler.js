...
export class OrderHandler {
    ...
    async handleBuyOrder(userSession, orderData) {
        const { volume, symbol = 'XAUUSD_TTBAR.Fix', phoneNumber } = orderData;
        
        try {
            console.log(`Processing BUY order for ${phoneNumber}: ${volume} ${symbol}`);

            // Step 1: Get current market price
            const marketData = await mt5MarketDataService.getMarketData(symbol, phoneNumber);
            if (!marketData) {
            ...
            const mt5TradeData = {
                ...
                type: 'BUY', // Market BUY
                comment: `BUY-${phoneNumber.slice(-4)}-${Date.now().toString().slice(-6)}`,
                magic: 123456
            };
            ...
            if (!mt5Result.success) {
                throw new Error(`MT5 BUY order failed: ${mt5Result.error || 'Unknown error'}`);
            }
            ...
            console.log('CRM BUY order stored successfully:', crmResult.clientOrder._id);
    ...
    async handleSellOrder(userSession, orderData) {
        ...
        try {
            console.log(`Processing SELL order for ${phoneNumber}: ${volume} ${symbol}`);
            ...
            const mt5TradeData = {
                ...
                comment: `SELL-${phoneNumber.slice(-4)}-${Date.now().toString().slice(-6)}`,
                magic: 123456
            };
            ...
            if (!mt5Result.success) {
                throw new Error(`MT5 SELL order failed: ${mt5Result.error || 'Unknown error'}`);
            }
            ...
            console.log('CRM SELL order stored successfully:', crmResult.clientOrder._id);
    ...
    async handleCloseOrder(userSession, orderData) {
        ...
        try {
            console.log(`Processing CLOSE order for ${phoneNumber}: Ticket ${ticket}`);

            // Step 1: Get current market price for profit calculation
            const marketData = await mt5MarketDataService.getMarketData('XAUUSD_TTBAR.Fix', phoneNumber);
            if (!marketData) {
            ...
            if (!mt5CloseResult.success) {
                throw new Error(`MT5 close failed: ${mt5CloseResult.error || 'Unknown error'}`);
            }
            ...
            return {
                ...
                message: `✅ Position Closed Successfully!\n\n` +
                        ...
                        `${profitEmoji} ${profitText}: ${Math.abs(mt5CloseResult.profit).toFixed(2)}\n` +
    ...
    async getUserPositions(userSession, phoneNumber) {
        try {
            ...
            const mt5Positions = await mt5MarketDataService.getOpenPositions(phoneNumber, phoneNumber);
...
export const integrateWithWhatsApp = async (body, userSession, phoneNumber) => {
    ...
    if (trimmedBody === 'positions' || trimmedBody === 'pos') {
        const result = await orderHandler.getUserPositions(userSession, phoneNumber);
        if (result.success) {
