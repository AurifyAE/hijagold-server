...
const DEFAULT_SYMBOL = 'XAUUSD_TTBAR.Fix';
...
const generateOrderNumber = () => {
    return `MT5-${Date.now()}-${Math.random().toString(36).substring(2, 7).toUpperCase()}`;
};
 ...
 */
export const processOrderPlacementMT5 = async (pendingOrder, userPhoneNumber) => {
    ...
    try {
        ...
        console.log(`Processing ${type} order for ${volume} grams for phone: ${phoneNumber}`);
        ...
        if (!mt5Service.isConnected) {
            try {
                ...
                return {
                    success: false,
                    message: "Unable to connect to trading platform. Please try again later.",
        ...
        try {
            ...
            await updateOrderStatus(tradeResult.clientOrder._id, {
                comment: mt5Result.ticket ? `MT5-${mt5Result.ticket}` : tradeData.comment,
...
async function getMarketDataWithFallback() {
    ...
    throw new Error(`Failed to get market data for any gold symbol. Last error: ${lastError?.message}`);
}
...
export const processOrderCloseMT5 = async (orderId, userPhoneNumber) => {
    try {
        ...
        if (order.mt5Ticket) {
            try {
                mt5CloseResult = await mt5Service.closeTrade(order.mt5Ticket, order.volume);
            } catch (mt5Error) {
 ...
 * @returns {Promise<Object>}
 */
export const processOrderPlacementMT5Enhanced = async (pendingOrder, userPhoneNumber) => {
    // Pre-check MT5 connection
    const connectionCheck = await validateMT5Connection();
    if (!connectionCheck.success) {
        return {
            success: false,
            message: "Trading platform is currently unavailable. Please try again in a few minutes.",
            error: ERROR_TYPES.MT5_CONNECTION_FAILED,
