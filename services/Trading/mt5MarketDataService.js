...
class OptimizedMT5MarketDataService {
  constructor() {
    this.symbols = ["XAUUSD.#"];
    this.symbolMapping = { GOLD: "XAUUSD.#" };
  ...
  async initializeMT5() {
    try {
      ...
      try {
        ...
        console.log("Available symbols containing XAU:", availableSymbols.filter(s => s.includes('XAU')));
        
        // Try to validate our symbol - now with proper URL encoding support
        const validatedSymbol = await mt5Service.validateSymbol("XAUUSD.#");
  ...
  async getMarketData(symbol = "XAUUSD.#", clientId = null) {
    if (clientId) {
    ...
    if (!data || !mt5Service.isPriceFresh(mapped, this.PRICE_CACHE_TTL)) {
      try {
        console.log(`Cache miss or stale data for ${mapped}, fetching fresh data`);
  ...
  async getOpenPositions(phoneNumber, clientId = null) {
    ...
    try {
      ...
      console.log(`Found ${filteredPositions.length} positions for ${phoneNumber}`);
