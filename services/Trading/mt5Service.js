...
dotenv.config();
const BASE_URL = "http://127.0.0.1:5000"; // Adjust to your Python API URL
...
class MT5Service {
  ...
  async placeTrade(tradeData, retryCount = 0) {
    ...
    try {
      ...
      const symbol = await this.validateSymbol(tradeData.symbol || "XAUUSD.#");
  ...
  getCachedPrice(symbol = "XAUUSD.#") {
    return this.priceData.get(symbol);
  }

  isPriceFresh(symbol = "XAUUSD.#", maxAge = 5000) {
    return (
  ...
  async testConnection() {
    try {
      ...
      const testResult = await this.getPrice("XAUUSD.#");
