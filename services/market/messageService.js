...
import { handleMainMenuMT5, handleVolumeInputMT5, handleOrderConfirmationMT5, handlePositionSelectionMT5 } from '../../controllers/chat/whatsappController.js';

export const getPriceMessageMT5 = async () => {
    try {
        const marketData = await mt5MarketDataService.getMarketData('XAUUSD_TTBAR.Fix');
        if (!marketData) return "⚠️ Unable to fetch gold prices. Please try again.";

        const timestamp = new Date(marketData.timestamp).toLocaleString('en-US', { timeZone: 'Asia/Dubai', dateStyle: 'short', timeStyle: 'medium' });
...
export const processUserInputMT5 = async (message, session, twilioClient, from, twilioNumber, phoneNumber) => {
    const input = message.trim().toLowerCase();
    console.log(`processUserInputMT5 called with input: ${input}, state: ${session.state}, phoneNumber: ${phoneNumber}`);
    ...
    try {
        switch (session.state) {
            ...
            case "CONFIRM_ORDER":
                console.log(`Routing to handleOrderConfirmationMT5 for input: ${input}`);
                return await handleOrderConfirmationMT5(input, session, phoneNumber);
            case "VIEW_POSITIONS":
                console.log(`Routing to handlePositionSelectionMT5 for input: ${input}`);
...
export const getPositionsMessageMT5 = async (session, phoneNumber) => {
    try {
        ...
        if (!positions.length) return `📊 Open Positions\n📝 No open positions.\n\nType MENU to return.`;
        ...
        positions.forEach((position, index) => {
            const profit = position.profit > 0 ? `+$${position.profit.toFixed(2)}` : `$${position.profit.toFixed(2)}`;
            message += `${index + 1}️⃣ Ticket ${position.ticket}\n   📊 ${position.type} ${position.volume} grams\n   💰 Open: $${position.price_open.toFixed(2)}\n   📈 Current: $${position.price_current.toFixed(2)}\n   📊 P&L: ${profit}\n\n`;
        });
